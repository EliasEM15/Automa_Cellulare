...
void NextGeneration (const vector<vector<int>>& CurrentGen, vector<vector<int>>& NextGen, int N)
{
        ...
        for(int x=1; x < N-1; ++x)
        {
            ...
            for(int dy=-1; dy <= 1; ++dy)
            {
                for(int dx=-1; dx <= 1; ++dx)
                {
                    ...
                    Alive += CurrentGen[y+dy][x+dx];

                }

            }
            int CurrentState= CurrentGen[y][x];
...
int main(int argc, char* argv[]){
    ...
    //griglia NxN
    vector<vector<int>> CurrentGen(N, vector<int>(N, 0));
    vector<vector<int>> Gen(N, vector<int>(N, 0));
    vector<vector<int>> NextGen(N, vector<int>(N, 0));
    for(int x=1; x < N-1; ++x)
    {
        for(int y=1; y < N-1; ++y)
        {
